...
-- Start: Added by Chris to split sending email address by @
...
-- End: Added by Chris to split sending email address by @
...
local function gen_auth_results(task, settings)
  ...
  -- Start: Added by Chris to extract sending domain
  ...
  -- End: Added by Chris to extract sending domain

  table.insert(hdr_parts, spoof_sender_hdr)
  ...
  if #dkim_results == 0 then
    ...
    if common.symbols[settings.dkim_symbols.none] then
      -- Start: Added by Chris to pass dkim in arc
      local spoof_dkim_hdr = string.format('dkim=pass header.d=%s', spoof_sender_hdr)
      table.insert(hdr_parts, spoof_dkim_hdr)
      -- End: Added by Chris to pass dkim in arc
    end
  end
  ...
  for auth_type, keys in pairs(auth_results) do
    for _, key in ipairs(keys) do
	  ...
	  -- Start: Added by Chris to pass spf in arc
	  comment = string.format('%s: domain of %s designates %s as permitted sender',
	       	spoof_sender_hdr, sender, tostring(task:get_from_ip() or 'unknown'))
	  -- Always pass
          hdr = string.format('%s=%s (%s) %s=%s', auth_type, 'pass',
              comment, sender_type, sender)
	  -- End: Added by Chris to pass spf in arc
        else
          hdr = string.format('%s=%s', auth_type, key)
        end
        
	-- Start: Added by Chris to pass dmarc in arc
	local dmarc_hdr = string.format('dmarc=pass action=none header.from=%s', spoof_sender_hdr)
	table.insert(hdr_parts, dmarc_hdr)
	-- End: Added by Chris to pass dmarc in arc
